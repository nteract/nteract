...
describe("empty live room displacement", () => {
  ...
  it("kicks one full materialization when the doc first catches up to the room", () => {
    assert.match(
      sessionSource,
      /observeCloudNotebookCatchUp\([\s\S]{0,200}?liveRuntime\.engine\.notebookSyncApplied\$,[\s\S]{0,100}?liveRuntime\.handle,[\s\S]{0,100}?\.subscribe\(\(caughtUp\) => \{[\s\S]{0,400}?if \(caughtUpMaterializeKicked\) return;\s*if \(!caughtUp\) return;\s*caughtUpMaterializeKicked = true;\s*materializeLiveCellsSafely\(liveRuntime\);/,
    );
  });

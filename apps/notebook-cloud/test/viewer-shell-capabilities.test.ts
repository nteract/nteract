...
test("cloud shell capabilities prefer the room-ready peer label over local auth fallbacks", () => {
  const capabilities = cloudNotebookShellCapabilities({
    ...
    connectionScope: "owner",
    connectionActorLabel: "user:anaconda:anaconda-user-123/browser:tab",
    connectionPeerLabel: "Kyle Kelley",
    hasAppSession: true,
    hasCodeCells: false,
  });

  assert.equal(capabilities.access.identityLabel, "Kyle Kelley");
  assert.equal(capabilities.access.actor?.principal.label, "Kyle Kelley");
  assert.equal(
    capabilities.access.actor?.actorLabel,
    "user:anaconda:anaconda-user-123/browser:tab",
  );
});

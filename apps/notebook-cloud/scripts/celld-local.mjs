...
//   NOTEBOOK_CLOUD_CELLD_PYTHON=1    opt in to the experimental private Python provider
...
function entryShim(worker, projectDir) {
  ...
  if (worker.name === "main") {
    if (process.env.NOTEBOOK_CLOUD_CELLD_PYTHON === "1") {
      const providerPath = path
        .relative(projectDir, path.join(appDir, "../preview-python/dist/provider.js"))
        .split(path.sep)
        .join("/");
      shimLines.push(`export { PreviewPythonSessions, PackageAssets } from "${providerPath}";`);
    }
...
async function copyProjectFiles(worker, projectDir) {
  ...
  if (worker.name === "main" && process.env.NOTEBOOK_CLOUD_CELLD_PYTHON === "1") {
    await writeFile(
      ...
      JSON.stringify({ version: 1, runtime: "pyodide-0.28.3" }),
    );
    await cp(
...
async function writeWorkerConfig(worker, projectDir, main, sessionSecret) {
  ...
  if (worker.name === "main" && process.env.NOTEBOOK_CLOUD_CELLD_PYTHON === "1") {
    config.vars.NOTEBOOK_CLOUD_PYTHON_PROVIDER = "celld";
    config.durable_objects.bindings.push({
      name: "PREVIEW_PYTHON_SESSIONS",
      class_name: "PreviewPythonSessions",
    });

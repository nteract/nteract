...
handle.update_source(
  ...
  [
    ...
    "",
    "- The .am snapshot is stored in R2.",
    "- The read-only render cache was generated by runtimed-wasm.",
  ].join("\n"),
);
...
const renderCache = {
  schema_version: 1,
  generated_from: "runtimed-wasm:get_cells_json",
  generated_at: new Date().toISOString(),
  notebook_id: notebookId,
  actor_label: actorLabel,
  heads,
  heads_hash: headsHash,
  cells,
};
...
await putBytes(
  ...
  {
    "X-Runtime-Heads-Hash": "none",
  },
);
await putText(
  `/api/n/${encodeURIComponent(notebookId)}/renders/${encodeURIComponent(headsHash)}`,
  JSON.stringify(renderCache, null, 2),
  "application/vnd.nteract.notebook.render+json; version=1",
);
...
assert(
  ...
  "latest render cache did not include the demo code cell",
);
  ...
  JSON.stringify(
    {
      ...
      cells: cells.length,
      checks: [
        "runtimed_wasm_notebookdoc_snapshot",
        "r2_snapshot_publish",
        "r2_render_cache_publish",
        "latest_render_readback",
      ],
    },
    null,
    2,
...
async function putText(pathname, body, contentType, extraHeaders = {}) {
  const response = await fetch(new URL(pathname, baseUrl), {
    method: "PUT",
    headers: publishHeaders(contentType, extraHeaders),
    body,

...
function NotebookViewer({ runtime }: { runtime: ViewerRuntime }) {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-5xl flex-col px-4 py-8 sm:px-6">
      ...
      <section className="mt-4 flex min-h-0 flex-1 flex-col" aria-label="Notebook cells">
...
function ReadonlyNotebookCell({
  ...
  return (
    <CellContainer
      ...
      gutterContent={<ExecutionPrompt cell={cell} />}
...
function ExecutionPrompt({ cell }: { cell: ResolvedCell }) {
  if (cell.cellType !== "code") {
    return (
      <span className="font-mono text-xs text-muted-foreground" title={cell.cellType}>
        {cell.cellType}
      </span>
    );
  }
  ...
  return (
    <span className="font-mono text-xs text-muted-foreground" title="Execution count">

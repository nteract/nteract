...
/**
 * Single owner of hosted-workstation state for the notebook viewer: the
 * registry snapshot, default/attach mutations, the cadence-driven refresh loop,
 * and the projections the rail and toolbar consume. Callers pass capabilities
 * and a panel-open signal and receive the gated callbacks already resolved, so
 * workstation state lives in one place instead of being threaded through the
 * viewer component.
 */
export function useCloudWorkstationManager({
  ...
  const handleSetDefaultWorkstation = useCallback(
    async (workstationId: string) => {
      ...
      setWorkstationMutation({ kind: "default", message: null, workstationId });
      try {

...
Selecting text raises a small floating toolbar beside the selection with a
single add-comment button. Clicking it opens the Discussions panel with the
selection staged as the draft target, so comments are always composed in the
panel and land in notebook order. The same toolbar appears in the CodeMirror

...
- The rail reserves a real toolbar-corner grid cell and starts navigation in
  the body row. The panel header and notebook toolbar use the same 40px command
  row plus its hairline separator, so the join is structural rather than
  visually offset into place.

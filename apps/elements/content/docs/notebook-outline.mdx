...
- The rail reserves a real toolbar-corner grid cell and starts navigation in
  the body row. Contextual panel headers and the notebook command toolbar use
  the same 40px command row plus its hairline separator, so the join is
  structural rather than visually offset into place.
- Outline omits a duplicate panel heading: its active icon and the
  ...
  label-free header track keeps the command-row hairline; only the fixed 56px
  rail corner remains borderless.

...
This fixture renders the shared `NotebookDocumentRail` and shared command
toolbar with scenario state from the [notebook shell capabilities](/docs/notebook-shell-capabilities)
fixtures. The package panel hosts the shared `UvDependencyPanel` in its rail
...
- The package panel imports `UvDependencyPanel` from `src/components/environment`
  with fixture package and sync state.

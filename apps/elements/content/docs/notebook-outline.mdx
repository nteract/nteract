...
- The rail supports an integrated toolbar-corner track and an external shell
  toolbar placement. The full shell uses the external placement: its outline
  toggle occupies the first 56px of the command row, while rail navigation and
  panel content begin in the body row. The shared grid makes the join structural
  rather than visually offset into place.
- Outline omits a duplicate panel heading: its active icon and the
  `Notebook outline` navigation label already identify the surface. Its
  shell toolbar carries the command-row hairline. The fixed 56px corner has no
  bottom or right rule; rail and panel dividers begin in the body row so the
  toolbar baseline turns cleanly into the vertical shell edge.

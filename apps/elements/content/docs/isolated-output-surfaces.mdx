...
content becomes shared output manifests. The catalog keeps that surface
runtime-free: docs fixtures can show the same decisions, but they do not serve
the daemon output document or execute generated renderer bundles.

The key production wrinkle is the hosted frame path added in #3142. When an MCP
host advertises the daemon blob origin in `frameDomains`, nteract uses the
daemon `/output-frame` document instead of relying on inline `srcdoc` inside the
host CSP. The rendered boundary map below calls out that split so future catalog
slices keep static fixtures separate from host-gated daemon frame behavior.
...
injection, daemon blob-route authorization, host `frameDomains` checks, and the
actual `/output-frame` document served from the daemon. The catalog should keep
those as explicit adapters until they can be exercised without importing desktop
runtime state or executing untrusted output code in the docs app.

...
Presence, room population, and host view/edit mode are intentionally absent
here; those belong to host/app chrome such as the cloud notebook shell.
The hosted scenarios are shared with the
[notebook shell capabilities](/docs/notebook-shell-capabilities) page so editor,
owner, runtime peer, delegated operator, and local desktop access do not drift
from the shared capability model.
...
command chrome, actor identity, runtime status projection,

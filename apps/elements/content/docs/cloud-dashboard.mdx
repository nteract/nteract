...
This fixture runs the real `projectCloudNotebookDashboard` projection from
`@/components/notebook/workspace`, the same one the hosted `/n` dashboard uses,
over deterministic notebooks. Ordering, the summary metrics, and the published /
access sidebar come from shared code, so the catalog cannot drift from
production behavior; only per-notebook display flavor stays local.

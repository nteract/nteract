...
The notebook-shaped output lane, MIME selection, widget-view output, traceback,
and Sift source facts come from the shared Elements scenario layer instead of
page-local mock state.
...
lane composition, ANSI streams and folded long-stream previews, ANSI errors,
JSON trees, image output, math output, SVG output, audio output, the JavaScript
isolation fallback, Plotly, Vega/Vega-Lite, GeoJSON, PDF, video, the structured
traceback renderer, and `SiftTable` with static `TableData` plus a live
HuggingFace Parquet URL. The page also renders a docs-served Arrow stream
manifest through `SiftTable`'s appendable WASM store. The page also renders top-level

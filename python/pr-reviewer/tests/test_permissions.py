...
def test_safe_bash_allows_read_only_git_and_search_commands() -> None:
    ...
    assert is_safe_bash_command("nl -ba pyproject.toml")
...
def test_safe_bash_blocks_shell_metacharacters_and_writes() -> None:
    ...
    assert not is_safe_bash_command("git diff --output=/tmp/diff HEAD")
    assert not is_safe_bash_command("git show -o /tmp/show HEAD")
    assert not is_safe_bash_command("python script.py")
    assert not is_safe_bash_command("find .")
    assert not is_safe_bash_command("find . -delete")
    assert not is_safe_bash_command("sed -n '1,120p' pyproject.toml")

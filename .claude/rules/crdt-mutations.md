...
5. **Do not mutate the doc directly after an async gap.** If you read doc state, await something, then write back, prefer `NotebookDoc::transact_at_heads_recovering(...)` with heads captured before the await. Use `fork_with_actor(...)` + `merge_recovering(...)` only when a forked document must cross the async gap. For synchronous blocks, prefer typed live-doc mutations or document-owned transaction helpers; keep `doc.fork_and_merge(|fork| { ... })` for older isolated-draft call sites. See `contributing/crdt-mutation-guide.md` for the full pattern.

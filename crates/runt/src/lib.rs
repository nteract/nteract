...
struct Cli {
    /// Select an installed nteract release channel
...
impl CliChannel {
    fn build_channel(self) -> runt_workspace::BuildChannel {
        match self {
            Self::Stable => runt_workspace::BuildChannel::Stable,
            Self::Nightly => runt_workspace::BuildChannel::Nightly,
        }
    }
...
/// Select the public command contract without inferring it from symlink names.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum EntryPoint {
    /// The global nteract CLI, including supervised MCP.
    Nteract,
    /// Compatibility entrypoint: `runt mcp` remains the direct worker.
    Runt,
}
...
fn cli_command(entrypoint: EntryPoint) -> clap::Command {
    ...
    match entrypoint {
        EntryPoint::Nteract => command
            ...
            .mut_subcommand("daemon", |command| {
                command.about("Manage this installation's runtime service")
            })
            .mut_subcommand("status", |command| {
                command.about("Show this installation's runtime service status")
            })
            .mut_subcommand("mcp", |command| {
                command.about("Serve MCP over stdio with automatic worker recovery")
            }),
        EntryPoint::Runt => command,
    }
}
...
enum Commands {
    ...
    /// Internal supervised MCP worker. Use `nteract mcp` in client configuration.
    #[command(name = "mcp-worker", hide = true)]
    McpWorker {
        #[arg(long)]
        no_show: bool,
        #[arg(long)]
...
/// Run either public CLI through the shared command implementation.
pub fn run(entrypoint: EntryPoint) -> Result<()> {
    enable_virtual_terminal_processing();

    if entrypoint == EntryPoint::Nteract {
        // Route before clap exits for --help/--version, so an explicit channel
        // reports the selected installation's CLI contract and version too.
    ...
    match cli.command {
        ...
        other => {
            let rt = tokio::runtime::Runtime::new()?;
            rt.block_on(async_main(other, entrypoint, cli.channel.is_some()))
        }
    }
}
...
fn channel_from_args(args: impl IntoIterator<Item = OsString>) -> Result<Option<CliChannel>> {
    ...
    while let Some(arg) = args.next() {
        let value = if arg == "--channel" {
            Some(
                args.next()
                    .ok_or_else(|| anyhow::anyhow!("--channel requires stable or nightly"))?,
            )
        } else if let Some(value) = arg.to_str().and_then(|arg| arg.strip_prefix("--channel=")) {
        ...
        if let Some(value) = value {
            if channel.is_some() {
                anyhow::bail!("--channel may only be provided once");
            }
            let value = value
                .to_str()
                .ok_or_else(|| anyhow::anyhow!("--channel requires stable or nightly"))?;
            channel = Some(CliChannel::from_str(value, false).map_err(anyhow::Error::msg)?);
        }
    }
    Ok(channel)
}

fn validate_channel_dispatch(
    requested: Option<CliChannel>,
    dispatched: Option<&str>,
    compiled: runt_workspace::BuildChannel,
) -> Result<()> {
    if let Some(dispatched) = dispatched {
        let requested = requested.ok_or_else(|| {
            anyhow::anyhow!("A channel-dispatched nteract invocation is missing --channel")
        })?;
        if dispatched != requested.as_str() || requested.build_channel() != compiled {
            anyhow::bail!(
                "The selected nteract backend does not match the requested {} channel; repair that CLI installation",
                requested.as_str()
            );
        }
    }
...
fn route_channel(requested: Option<CliChannel>) -> Result<()> {
    let dispatched = std::env::var(CHANNEL_DISPATCH_ENV).ok();
    validate_channel_dispatch(
        requested,
        dispatched.as_deref(),
        runt_workspace::build_channel(),
    )?;
    // Do not let the one-hop validation marker leak into notebook processes or
    // the MCP worker (which deliberately has no public --channel argument).
    std::env::remove_var(CHANNEL_DISPATCH_ENV);
    let Some(requested) = requested else {
        return Ok(());
    };
    if requested.build_channel() == runt_workspace::build_channel() {
        return Ok(());
    }
    let executable = runt_workspace::cli::resolve_channel(requested.build_channel())
        .map_err(anyhow::Error::msg)?;
    let mut command = std::process::Command::new(executable);
    command
        .args(std::env::args_os().skip(1))
        .env(CHANNEL_DISPATCH_ENV, requested.as_str());
    #[cfg(unix)]
    {
...
async fn async_main(
    ...
    match command {
        ...
        Some(Commands::Inspect {
            ...
            full_outputs,
            json,
        }) => inspect_notebook(&path, full_outputs, json, local_runtime).await?,
        ...
        Some(Commands::Workstation { command }) => {
            workstation_cli::command(command, entrypoint == EntryPoint::Nteract).await?
        }
        Some(Commands::Config { command }) => config_command(command).await?,
        Some(Commands::Mcp { no_show, socket }) if entrypoint == EntryPoint::Nteract => {
            let executable = std::env::current_exe()?;
            let code = nteract_mcp::run_unified(executable, no_show, socket, explicit_channel)
                .await
                .map_err(anyhow::Error::msg)?;
            if code != std::process::ExitCode::SUCCESS {
                anyhow::bail!("MCP supervisor could not complete successfully");
            }
        }
        Some(Commands::Mcp { no_show, socket }) => {
            ...
            run_mcp_server(no_show, local_runtime).await?
        }
        Some(Commands::McpWorker {
            no_show,
            socket,
            explicit_channel,
        }) => {
            ...
            run_mcp_server(
                no_show,
                local_runtime::LocalRuntimeOptions {
                    explicit_channel,
                    ..local_runtime
                },
            )
        ...
        None => {
            let mut command = cli_command(entrypoint);
            if entrypoint == EntryPoint::Runt {
                command = command.about(random_tagline());
            }
            command.print_help()?;
        }
    }
...
async fn run_mcp_server(
    no_show: bool,
    local_runtime: local_runtime::LocalRuntimeOptions,
) -> Result<()> {
    ...
    // ── Server setup ─────────────────────────────────────────────────
    let selected_runtime = local_runtime::select(local_runtime, None).await;
...
async fn notebook_client(
    options: local_runtime::LocalRuntimeOptions,
) -> Result<runtimed::client::PoolClient> {
    use runtimed::client::PoolClient;
    use runtimed_client::singleton::query_daemon_info;
    if options.enabled {
        let selected = local_runtime::select(options, None).await;
        let info = runtimed_client::startup::probe_local_runtime(&selected.endpoint).await?
            .ok_or_else(|| anyhow::anyhow!("No notebook runtime is listening at {}. Connect or create a local notebook to start it.", selected.endpoint.display()))?;
        return Ok(PoolClient::new(PathBuf::from(info.endpoint)));
    }
    Ok(
        match query_daemon_info(runt_workspace::default_socket_path()).await {
            Some(info) => PoolClient::new(PathBuf::from(&info.endpoint)),
...
async fn list_notebooks(
    ...
    let client = notebook_client(options).await?;
...
mod entrypoint_tests {
    ...
    #[test]
    fn canonical_help_has_one_name_and_hides_worker() {
        let help = cli_command(EntryPoint::Nteract).render_help().to_string();
        assert!(help.contains("Usage: nteract"));
        assert!(help.contains("workstation"));
        assert!(help.contains("mcp"));
        assert!(help.contains("--channel"));
        assert!(!help.contains("mcp-worker"));
        assert!(!help.contains("runt:"));
    }

    #[test]
    fn worker_and_public_mcp_accept_the_same_options() {
        for command in ["mcp", nteract_mcp::WORKER_COMMAND] {
            let matches = cli_command(EntryPoint::Nteract)
            ...
            match cli.command.unwrap() {
                Commands::Mcp { no_show, socket }
                | Commands::McpWorker {
                    no_show, socket, ..
                } => {
                    assert!(no_show);
                    assert_eq!(socket, Some(PathBuf::from("/tmp/selected.sock")));
                }
    ...
    #[test]
    fn channel_dispatch_checks_the_actual_backend_identity() {
        use runt_workspace::BuildChannel;
        assert!(validate_channel_dispatch(
            Some(CliChannel::Nightly),
            Some("nightly"),
        ...
        .is_err());
        assert!(validate_channel_dispatch(None, Some("nightly"), BuildChannel::Nightly).is_err());
    }

    #[test]
    fn channel_routing_only_reads_top_level_options() {
        let parse = |args: &[&str]| channel_from_args(args.iter().map(OsString::from));
        assert_eq!(
            parse(&["--channel", "nightly", "--version"])
                .unwrap()
                .unwrap()
                .as_str(),
            "nightly"
        ...
        assert_eq!(
            parse(&["--channel=stable", "mcp"])
                .unwrap()
                .unwrap()
                .as_str(),
            "stable"
        );
        assert!(parse(&["nb", "call", "--channel", "nightly"])
            .unwrap()
            .is_none());
        assert!(parse(&["--channel", "unknown"]).is_err());
        assert!(parse(&["--channel"]).is_err());
        assert!(parse(&["--channel", "stable", "--channel", "nightly"]).is_err());
    }
}

...
pub(super) async fn handle_comments_doc_frame(
    ...
    let reply_encoded = room.comments.with_doc(|comments_doc| {
        if has_client_changes {
            ...
            let mut preview_base = comments_doc.doc().clone();
            let mut preview = CommentsDoc::from_doc(preview_base.clone())?;
            ...
            match preview.receive_sync_message_with_changes_recovering(
                ...
                Ok(true) => {
                    let actors: std::collections::BTreeSet<String> = preview_base
                        .get_changes_added(preview.doc_mut())
                        .into_iter()
                    ...
                    connection_identity
                        .validate_actor_labels(actors.iter().map(String::as_str))

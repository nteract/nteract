export const FRAME_HTML = String.raw`<!DOCTYPE html>
<html style="background:transparent;color-scheme:dark">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta http-equiv="Content-Security-Policy" content="default-src 'self' blob: data:; script-src 'unsafe-inline' 'unsafe-eval' blob: https: http://127.0.0.1:*; style-src 'unsafe-inline' https: http://127.0.0.1:*; img-src * data: blob:; font-src * data:; media-src * data: blob:; object-src * data: blob:; connect-src *; worker-src 'self' blob:; frame-src 'none'; child-src 'none';">
  <style>
    :root {
 ...
 *
 * Tauri loads this HTML through the nteract-frame custom scheme so the frame
 * receives its own CSP. Browser-only development keeps using srcDoc with the
 * same sandbox-induced opaque origin behavior.
 */

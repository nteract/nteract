...
export interface CrdtSourceHandle {
  get_cell_source(cellId: string): string | null | undefined;
  splice_source(cellId: string, index: number, deleteCount: number, text: string): boolean;
  update_source(cellId: string, source: string): boolean;
}

export interface CrdtBridgeConfig {
  /** Read the current source handle (null during bootstrap). */
  getHandle: () => CrdtSourceHandle | null;
...
export function createCrdtBridge(config: CrdtBridgeConfig): CrdtBridge {
  ...
  class CrdtBridgePlugin implements PluginValue {
    ...
    update(vu: ViewUpdate) {
      ...
      const outboundTxs = vu.transactions.filter((tr) => tr.docChanged && !isReconcileTx(tr));
      ...
      try {
        ...
        for (const tr of outboundTxs) {
          ...
          tr.changes.iterChanges(
            (fromA: number, toA: number, _fromB: number, _toB: number, inserted) => {
              changes.push({
                fromA,
                toA,
            ...
            if (hasNonBMP) {
              ...
              logger.debug("[crdt-bridge] SPLICE", {
                ...
                nearbyChars: JSON.stringify(
                  docText.slice(Math.max(0, fromA - 2), fromA + deleteCount + 2),
                ),
                nearbyCodePoints: Array.from(
                  docText.slice(Math.max(0, fromA - 2), fromA + deleteCount + 2),

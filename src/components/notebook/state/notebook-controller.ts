...
export type NotebookControllerMutationKind = "source" | "structure" | "outputs" | "visibility";
...
export interface NotebookControllerOptions<THandle extends NotebookControllerHandle> {
  ...
  fallbackCell?: (cellId: string, cellType: NotebookControllerCellType) => NotebookCell | null;
  logPrefix?: string;
}
...
export function createNotebookController<THandle extends NotebookControllerHandle>({
  ...
  return {
    ...
    addCell(cellType, afterCellId = null) {
      ...
      return (
        getNotebookCellsSnapshot().find((cell) => cell.id === cellId) ??
        fallbackCell(cellId, cellType)
      );
    },

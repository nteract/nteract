...
describe("managed Python package controls", () => {
  it("shows included versions before compute starts without claiming requested packages are installed", () => {
    render(<ManagedPythonPackages {...props} phase="unavailable" installed={[]} />);
    expect(screen.getByRole("region", { name: "Included with Python" })).toHaveTextContent("numpy");
    expect(screen.getByRole("region", { name: "Included with Python" })).toHaveTextContent("2.2.5");
    expect(screen.getByRole("region", { name: "Saved requirements" })).toHaveTextContent("six");
    expect(screen.queryByRole("region", { name: "Installed additions" })).toBeNull();
  ...
  it("keeps failed state and installed observations separate from saved requirements", () => {
    ...
    expect(screen.getByText("Installed additions (1)")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Install" })).toBeDisabled();

...
export function ManagedPythonPackages({
  ...
  const addedInstalled = installed.filter((spec) => !includedSet.has(spec));
  ...
  return (
    <div className="space-y-4 text-xs" data-slot="managed-python-packages">
      ...
      {addedInstalled.length > 0 && (
        <section aria-label="Installed additions" className="space-y-2 border-t border-border pt-3">
          <h3 className="font-medium">Installed additions ({addedInstalled.length})</h3>
          <p className="leading-5 text-muted-foreground">
            Includes dependencies added for your packages.
          </p>
          <PackageSpecList
            values={addedInstalled}
            emptyLabel="No installed additions."
            framed={false}
          />
        </section>
      )}
      <section aria-label="Included with Python" className="space-y-2 border-t border-border pt-3">
        <h3 className="font-medium">Included with Python ({included.length})</h3>
        <p className="leading-5 text-muted-foreground">
          These packages are available in every fresh Python session. Import them in a code cell.
        </p>
        <PackageSpecList
          values={included}
          emptyLabel="Included package details are unavailable."
          framed={false}
        />
      </section>

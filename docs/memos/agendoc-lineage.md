...
**Refreshed:** 2026-07-01 against `origin/main`, current nteract docs, and
current Automerge / Automerge Repo release state.
...
The direction of the repo since this memo was first drafted strengthens the
Agendoc shape:

- `CommentsDoc` is now an accepted sidecar direction with implementation in
  progress, its own sync stream, identity, projection, persistence, attachment,
  fan-out, and publish policy (`docs/adr/notebook-comments-document.md:8`). It
  also changed the trust story: comments are plain authored Automerge changes,
  and attribution is projected from admitted change actors rather than stored in
  trusted document fields (`docs/adr/notebook-comments-document.md:14`).
- Hosted revision rows now include comments coordinates as well as runtime and
  comms coordinates: `comments_heads_hash` and `comments_snapshot_key`
  (`apps/notebook-cloud/src/storage.ts:19`,
  `apps/notebook-cloud/src/storage.ts:158`).
  This makes the hosted snapshot bundle model more obviously multi-document.
- The document split ADR now explicitly says the document count is not the
  concept. Room documents, daemon-scoped documents, and sidecars split on
  authority, durability, attachment identity, and fan-out scope
  (`docs/adr/document-split.md:3`, `docs/adr/document-split.md:42`). Agendoc
  should follow the same rule.
- The schema evolution ADR is sharper about structural change: freeze the
  genesis, layer additive fields, and put structural changes into sidecar
  documents instead of reshaping a live root (`docs/adr/schema-evolution-and-genesis.md:36`).
- The shared-store projection memo records the current UI direction: source
  documents feed projection-backed shared stores, while host-specific transport
  remains outside those stores (`docs/memos/shared-store-projection-convergence.md:7`).
...
heads, optional comms heads, optional comments heads, snapshot keys, and actor
labels (`apps/notebook-cloud/src/storage.ts:19`). The hosted artifacts ADR says durable
...
before recording a durable revision (`apps/notebook-cloud/src/index.ts:2676`,
`apps/notebook-cloud/src/index.ts:3923`). For Agendoc, the analogous durable
...
Automerge's current modeling guide gives the same warning at the CRDT level:
fine-grained mutations merge better than replacing whole objects, and a document
is best treated as a collaboration unit rather than a dumping ground for
unbounded unrelated state. It also warns that many thousands of documents have
sync overhead, while hundreds are a reasonable scale to consider. That supports
Agendoc as one collaboration unit with sidecars for authority or retention
boundaries, not one document per tool event.
...
### Current Upstream Automerge Read

As of 2026-07-01, `@automerge/automerge` latest is `3.2.6`; the `next` tag is
`3.3.0-fragments.2`. Automerge 3.0's key architectural change is much lower
loaded-document memory usage while keeping the Automerge 2 file format nearly
compatible. That makes long-lived Agendoc sessions more plausible than they
were under Automerge 2, but it does not remove the need for explicit lineage
compaction: Automerge still stores full history by design, and product-level
compaction here means deriving a new document with recorded provenance, not
garbage-collecting history in place.

`@automerge/automerge-repo` latest is `2.5.6`, with `2.6.0-alpha.2` and a
`2.6.0-subduction.*` line active. Repo remains useful as a reference for
multi-document management, pluggable storage, and multiple network adapters.
It is not a drop-in replacement for nteract's typed-frame protocol: nteract
already splits documents by authority, uses request-scoped causal gates such as
`required_heads`, and needs hosted ACL/runtime-peer policy at the room host.

The Rust `automerge-repo-rs` project is also not a shortcut here: its README
says its filesystem layout and WebSocket sync protocol are not compatible with
the JavaScript automerge-repo implementation. The relevant lesson is still the
same: Agendoc should have a clean document/repo abstraction boundary, but the
harness protocol and hosted authority model remain nteract-specific until a
real compatibility target is chosen.

External sources checked:

- Automerge 3.0 announcement:
  `https://automerge.org/blog/automerge-3/`
- Automerge modeling guide:
  `https://automerge.org/docs/cookbook/modeling-data/`
- Automerge lists guide:
  `https://automerge.org/docs/reference/documents/lists/`
- Automerge Repo docs:
  `https://automerge.org/docs/reference/repositories/`
- Automerge Repo networking docs:
  `https://automerge.org/docs/reference/repositories/networking/`
- `npm view @automerge/automerge`, `npm view @automerge/automerge-repo`, and
  `npm view @automerge/automerge-repo-sync-server`, checked 2026-07-01.
...
### 7. Keep Schema Evolution Boring

If Agendoc lives in this repo's Automerge stack, it should copy the current
schema-evolution posture rather than invent a looser one:

- create a canonical genesis seed for the root skeleton;
- treat schema bumps as layered additive writes;
- put structural changes into sidecars or derived documents;
- tolerate newer additive fields without downgrading them; and
- keep compaction as a lineage transform into a new document, not an in-place
  history rewrite.

That keeps "ordinary work mutates the current document" separate from "a
transform produced a new document shape." It also gives phones, desktops,
harnesses, and hosted rooms a common starting point for sync without asking
every peer to recreate identical root objects independently.
...
CommentsDoc is now the concrete precedent: it is separate because comments have
their own identity, sync stream, projection, persistence, attachment, fan-out,
and publish policy.
...
OC-10: Genesis and migration

Agendoc needs an explicit genesis and migration policy before becoming a durable
sync format. The default should mirror the notebook schema rule: freeze the
genesis, make schema bumps additive, tolerate future fields, and use sidecars or
derived documents for structural changes.

...
```bash
cargo run -p runt -- publish --vanity-name topic-viz ~/notebooks/topic-viz.ipynb
```

The notebook id is generated by the remote host: the publisher reserves a
publish target and reads the `notebook_id` (and viewer URL) back from the
response. There is no `--id` flag; `--vanity-name` only adds the optional
vanity path segment to the reported viewer URL.

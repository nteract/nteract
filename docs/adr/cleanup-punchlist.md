...
| HCA-5 | Blob hash representation is not a single contract across local CAS, blob-ref manifests, hosted artifact examples, and cloud storage keys. Choose bare lowercase SHA-256 hex, `sha256:<hex>`, or another durable syntax, then update examples and validators together. | Design | `docs/adr/blob-storage-and-content-addressing.md`, `docs/adr/blob-ref-and-chunk-manifest-protocol.md`, `docs/adr/hosted-notebook-artifacts.md`, `apps/notebook-cloud/src/storage.ts` |

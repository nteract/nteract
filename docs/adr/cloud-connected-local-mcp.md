...
1. Add a target parser with tests for local paths, local ids, hosted URLs, and
   `notebook_id` plus hosted `domain`.
2. Add a machine-local cloud domain registry reader. Store only non-secret
   routing and credential references.
3. Add a cloud session connector using `notebook-cloud-transport` and either
   the configured credential or a hosted sync ticket minted from it.
4. Introduce the smallest session-handle abstraction needed so existing MCP
   tools can operate over local and hosted document sets.
5. Extend `connect_notebook` with `target` while preserving `path` and
   `notebook_id`.
6. Add hosted list support to `list_notebooks(domain?: string)` as a separate
   source from local active rooms.
7. Extend proxy rejoin handoff to preserve encoded remote targets.
8. Add integration tests using a local hosted-room test server before relying on
   preview smoke tests.

## Open Questions

1. Should the registry path be `cloud-domains.toml`, `cloud-remotes.toml`, or
   part of a broader machine-local `connections.toml`?
2. Should a later version add aliases such as `preview:01KT...`, or is
   `domain` explicit enough?
3. Should operator suffixes default from MCP client branding plus host name, or
   require explicit config?
4. How should Desktop expose the same domain registry without making cloud auth
   look like normal synced app settings?
5. Which hosted catalog endpoint should back remote notebook listing, and what
   fields are safe to expose to agents by default?

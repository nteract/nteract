...
**Neighbors:**
- `docs/architecture/typed-frame-v4-wire-protocol.md` - the wire that backs every `DocHandle` the MCP server holds.
- `docs/architecture/three-document-split.md` - what `NotebookSession.handle` actually points at (`NotebookDoc`, `RuntimeStateDoc`, plus the runtime broadcast).
- `docs/architecture/execution-pipeline.md` - why a stale `DocHandle` is so painful for the agent: `required_heads`, output sync, and broadcast replay all run through it.
- `docs/architecture/blob-storage-and-content-addressing.md` - the blob HTTP port lives on the same `Daemon` the MCP proxy supervises.
- `docs/architecture/identity-and-trust.md` - the principal/operator model the proxy/child will eventually enforce per connection; today the MCP child connects as `local:<uid>` via peer creds.
- `docs/architecture/cleanup-punchlist.md` - parking lot for smells uncovered while drafting this ADR.
...
6. **Cross-daemon proxy resumption.** Proxy stamps the reconnection banner from `(old_daemon_version, new_daemon_version)`. If the daemon socket *path* changes (e.g., dev-mode worktree switch in isolated mode), the proxy treats it as a child crash, restarts the child, and the new child sees a fresh daemon. The proxy's `last_notebook_id` is meaningless in the new daemon's room space. There is no current check for this; the rejoin will fail and `SessionDropReason::Evicted` (or `Disconnected`) is recorded.

...
| TMD-1 | The Tokio mutex lint at `crates/runtimed/tests/tokio_mutex_lint.rs:18` uses `std::fs::read_dir` (no recursion). Only top-level files in `crates/runtimed/src/*.rs` are scanned. The 24-file `notebook_sync_server/` subdirectory plus `runtime_agent/`, `requests/`, etc. are silently invisible. The "zero violations" claim is scoped to top-level files only. One-line fix: switch to `walkdir`. | Targeted PR | `crates/runtimed/tests/tokio_mutex_lint.rs` |
...
- **Targeted PRs (one per smell):** WP-6, WP-7, WP-11, 3D-1, 3D-2, EP-1, EP-2, EP-8, EP-10, EP-11, BS-1, BS-3, BS-7, TMD-1, TMD-2, MSL-1, FSB-1, FSB-2. Eighteen open. (WP-2 and WP-3 landed in stacks 2 and 3.)

...
| ID | Smell | Severity | Where |
|----|-------|----------|-------|
| HCA-1 | Mixed hosted credentials are documented as reject-by-default in `hosted-credential-transport.md`, but the current notebook-cloud prototype prefers a forwarded `Cf-Access-Jwt-Assertion` over client-carried `CF-Access-Token` / bearer headers. Reject mixed identity-bearing credentials unless a deployment proves they are one credential. | Targeted PR | `apps/notebook-cloud/src/identity.ts`, `apps/notebook-cloud/test/identity.test.ts`, `docs/architecture/hosted-access-anaconda-demo-runbook.md` |
| HCA-2 | `editor` and `runtime_peer` are documented as orthogonal scopes, but cloud authorization currently lets `editor` include runtime-write capability while a later materializer gate narrows runtime-peer behavior again. Decide whether non-owner role requests require exact ACL rows or whether scopes are capability subsets, then align Rust identity, cloud authorization, and ADRs. | Design | `apps/notebook-cloud/src/authorization.ts`, `apps/notebook-cloud/src/room-materializer.ts`, `crates/nteract-identity/src/lib.rs`, `docs/architecture/hosted-room-authorization.md` |
| HCA-3 | Editor blob-upload authority is unresolved. The runtime/blob audit allows editor uploads as precursors to authorized references, while current cloud code and README deny editor uploads. Staged policy should be explicit: editors cannot upload until server-side reference-path validation lands, or editor upload and path validation must ship together. | Design | `apps/notebook-cloud/src/identity.ts`, `apps/notebook-cloud/README.md`, `docs/architecture/runtime-peer-and-blob-authority-audit.md` |
| HCA-4 | `identity-and-trust.md` is accepted and describes principal-prefixed schema-seed actors, but current `notebook-doc` and `runtime-doc` still use legacy `nteract:*schema*` actors. Actor-principal validators need explicit legacy/system exceptions unless the schema actor migration lands. | Design | `crates/notebook-doc/src/lib.rs`, `crates/runtime-doc/src/doc.rs`, `docs/architecture/identity-and-trust.md` |

...
- `runtime_peer` - permitted to write `RuntimeStateDoc` execution/output/lifecycle state. Cannot edit `NotebookDoc`. Used by future remote-runtime services and by JupyterHub-spawned runtime sidecars/services when they connect a kernel. The hosted Worker prototype exercises this as direct `0x05` sync from a runtime authoring handle; it still does not host kernels inside the Durable Object.

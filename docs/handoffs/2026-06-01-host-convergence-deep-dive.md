...
RuntimeStateDoc. Comm *state* (mutable trait values) moves to CommsDoc. Before
the split, RuntimeStateDoc policy enforced the topology/state boundary inside a
single document; current code has removed that editor carve-out and rejects
non-runtime-peer RuntimeStateDoc deltas wholesale. The load-bearing subtlety:
legacy orphan-comm state writes silently no-op because
`set_comm_state_property` bails `Ok` when the comm entry is absent - call it
**Guard A**. Guard A only works when topology and state are co-located.
Multi-principal CommsDoc writes remove that co-location, so safety rests on the
runtime before-snapshot membership gate that drops orphan state before forwarding
it to the kernel.

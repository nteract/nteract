 ...
 *
 * Videos only exist for runs made in recording mode (NTERACT_E2E_RECORD=1).
 *
 ...
 *
 * Each app wires this up in a small `e2e/show-video.mjs` shim that passes its
 * own resultsDir/recordingsDir and record command; see
...
const viewerHtml = path.join(path.dirname(fileURLToPath(import.meta.url)), "video-viewer.html");
...
/**
 * Archive, encode, and open the newest matching recording. Exits the process
 * non-zero when there is no video to show.
 *
 * @param {object} options
 * @param {string} options.resultsDir Playwright output dir to search.
 * @param {string} options.recordingsDir Persistent dir to archive encoded clips into.
 * @param {string} options.recordCommand Command that produces videos, for the error hint.
 * @param {string} [options.pattern] Substring of the spec/dir name.
 */
export function showVideo({ resultsDir, recordingsDir, recordCommand, pattern }) {
  const found = newestResultDir(resultsDir, pattern, (f) => f === "video.webm");
  if (!found) {
  ...
  const trimStart =
    Number.isFinite(meta.trimSeconds) && meta.trimSeconds > 0 ? meta.trimSeconds : 0;
  ...
  console.log(`Done. Video: ${outPath}`);
  console.log(`Kept in ${displayPath(recordingsDir)}/ — prior recordings preserved.`);
  openViewer(outPath);
}
